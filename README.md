  ...
  default:
    - step:
        ...
        script:
          ...
          - |
            if [[ "$BITBUCKET_BRANCH" == "main" ]]; then # Bitbucket uses master by default, adjust if your default branch is different
              tag=":latest"
              echo "Running on default branch '$BITBUCKET_BRANCH': tag = 'latest'"
            else
...
This example assumes two secrets to be available to authenticate against Docker Hub, called `DOCKER_HUB_USER` and `DOCKER_HUB_PAT`, beyond this, we need two repository variables called `CI_REGISTRY`, `CI_REGISTRY_IMAGE` about registry info and the last called `DOCKER_BUILDKIT` value should equal a 0 to avoid conflicts in Bitbucket architecture. 
